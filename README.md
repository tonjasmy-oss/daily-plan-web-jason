...
- ✅ **完整业务模块**：登录 / 仪表盘 / 项目管理 / 任务看板 / 任务详情 / 人员管理 / 部门管理 / 角色权限 / 系统参数 / 个人中心 / 周计划填报 / 计划周报 / 报表浏览 / 历史记录 / 审批管理 / 物资管理
...
├── approval.html          # 审批管理（用户审批 + 日计划/周计划/周报表单审批）
├── purchase-mgmt.html     # ★ 物资管理（物资申购审批独立模块，与其他审批分开）
├── projects.html          # 项目列表
├── project.html           # 项目详情
...
- 导出文档的表格排版：序号列窄而居中；日计划「人员安排」的「**类型**」列定宽且**不换行**
  （否则会被表宽挤成一字一行的竖排），剩余宽度让给「人员名单」列——名单过长时由该列自动换行，
  类型列始终保持单行
...
```
填报人提交 ──▶ 待审批(pending) ──┬─▶ 审批人追加工作内容（可多条，可逐条移除）
                                ├─▶ 通过 ──▶ 已通过(approved) ──▶ 记录锁定，只能查看
                                └─▶ 驳回 ──▶ 已驳回(rejected) ──▶ 填报人修改后重新提交
```

- **周计划填报也走审批**：提交时写入 `status='pending'` + 提交人 + 提交时间，
  提交后依旧清空表单并顺延到下一周，不回显已提交内容
- 审批人 = 主管以上（`admin` / `manager`），与日计划填报页原有规则一致；
  普通填报人即使处于待审批状态也看不到追加入口

### 审批人在哪里操作

| 记录 | 追加入口 | 审批入口 |
|---|---|---|
| 日计划 | 日计划填报页（任务区标题旁 + 底部）+ 报表浏览详情 | 同上两处 |
| 周计划 | 报表浏览 → 周计划 → 查看 | 报表浏览 → 周计划 → 查看 |

- 报表浏览详情里的审批区**内联**在弹层内（`追加工作内容` / `驳回原因` 两栏并排，
  `✓ 通过审批` 单独置底），主操作在首屏内可直接点击，无需滚动
- 之所以不用通用确认弹窗：`confirmDialogEx` / `promptDialog` 打开时会清空页面里**所有**
  `.modal-overlay`，在详情弹层之上再开弹窗会把详情一起销毁

### 追加条目的数据结构与呈现

- 追加条目写入任务的 `appended: true` + `appended_by` + `appended_at`，并**同时写 `content` 与 `title`**
  —— 日计划任务字段名是 `content`、周计划是 `title`，只写一个会导致另一边渲染不出来
- 日计划填报页里追加条目**单独紧凑渲染**：序号位显示「补」、左侧琥珀色描边、
  「审批追加」标签 + 追加人 / 时间；只读，且通过前可逐条移除
- 追加条目**不参与必填校验**（工作要求 / 实施人员 / 计划完成时间由原计划承担）
- 报表浏览详情、图片 / PDF / Excel 导出都会带上追加内容

### 只读约束

- 审批通过后：填报页字段全部禁用、无追加入口与审批按钮；报表浏览详情无审批区，
  周计划也不提供「打开填报页」入口 —— 只能查看
- 未通过的记录在报表浏览里**仍然可见**（带状态标签），便于审批人集中处理；
  日计划与周计划的导出均要求「已通过」（未通过时三个导出按钮置灰并给出原因）
